...
\begin{tabularx}{\textwidth}{Z{0.21\textwidth}Z{0.12\textwidth}Z{0.22\textwidth}Y}
\toprule
...
Watch values &
\texttt{watch=\{"ap.heading":0.25,"ap.enabled":true\}} \\
\bottomrule
\end{tabularx}
\end{table}
...
& \text{compass/GPS/NAV modes},\\[4pt]
\operatorname{resolv}(\psi_{\mathrm{input}},0^\circ),
& \text{wind-relative modes}.
\end{cases}
\]
...
\ttfamily
watch=\{"values":true\}\\
watch=\{"ap.enabled":true,"ap.mode":true,"ap.heading":0.5,"ap.heading\_command":true\}\\
ap.mode="compass"\\
...
\begin{quote}
\ttfamily
values=\{"ap.enabled":\{"type":"BooleanProperty","writable":true\},...\}\\
ap.mode="compass"\\
ap.heading\_command=90.0\\
ap.enabled=true\\
ap.heading=88.7000\\
...
\[
\begin{array}{ll}
1. & \text{open a TCP connection to } \texttt{host:23322},\\
2. & \text{send newline-terminated assignment records},\\
3. & \text{parse incoming lines by splitting at the first } \texttt{=},\\
4. & \text{decode the right-hand side as JSON},\\
5. & \text{handle } \texttt{error} \text{ records separately},\\
6. & \text{use } \texttt{watch} \text{ subscriptions for asynchronous updates}.
\end{array}
\]
...
\begin{table*}[t]
\centering
\caption{Basic pilot gains.}
\label{tab:pypilot-basic-gains}
\begin{tabularx}{\textwidth}{Z{0.25\textwidth}ccZ{0.18\textwidth}Y}
\toprule
...
\begin{table*}[t]
\centering
\caption{Absolute pilot gains.}
\label{tab:pypilot-absolute-gains}
\begin{tabularx}{\textwidth}{Z{0.25\textwidth}ccZ{0.18\textwidth}Y}
\toprule
Value & Default & Code range & Practical range & Meaning \\
\midrule
\texttt{ap.pilot.absolute.P} &
\num{0.05} &
\(\num{0}\ldots\num{0.3}\) &
\(\num{0.02}\ldots\num{0.12}\) &
...
\texttt{imu.headingraterate\_lowpass\_constant} &
\num{0.1} &
\(\num{0.05}\ldots\num{0.3}\) &
\(\num{0.05}\ldots\num{0.15}\) &
...
\begin{table*}[t]
\centering
\caption{GPS and APB navigation parameters.}
\label{tab:pypilot-gps-apb-parameters}
\begin{tabularx}{\textwidth}{Z{0.25\textwidth}ccZ{0.18\textwidth}Y}
\toprule
...
\subsection{Optional GPS Filter Parameters}
...
\begin{table}[h]
\centering
\caption{User-visible GPS filter values.}
\label{tab:pypilot-gps-filter-values}
\begin{tabularx}{\columnwidth}{Z{0.35\columnwidth}ccY}
\toprule
Value & Default & Practical range & Meaning \\
\midrule
...
\end{tabularx}
\end{table}
The fixed measurement and covariance constants are summarized in
\cref{tab:pypilot-gps-filter-fixed}.
\begin{table}[h]
\centering
\caption{Fixed GPS filter constants.}
\label{tab:pypilot-gps-filter-fixed}
...
\(\SI{10}{m}\) &
GPS position noise assumption. \\
Velocity measurement sigma &
\(\SI{0.25}{m/s}\) &
GPS velocity noise assumption. \\
Initial position deviation &
\(\SI{30}{m}\) &
Initial/process covariance scale. \\
Initial velocity deviation &
\(\SI{3}{m/s}\) &
Initial/process covariance scale. \\
3D mode &
\texttt{false} &
The filter runs in 2D by default. \\
\bottomrule
\end{tabularx}
\end{table}
...
\end{tabularx}
\end{table}
\subsubsection{Current, Voltage, and Power Filters}
\begin{table*}[t]
\centering
\caption{Servo electrical calibration and filtering.}
\label{tab:pypilot-servo-electrical}
\begin{tabularx}{\textwidth}{Z{0.25\textwidth}ccZ{0.18\textwidth}Y}
\toprule
...
\texttt{servo.compensate\_voltage} &
\texttt{false} &
boolean &
drive-dependent &
If enabled, scales command approximately by \(12/V\). \\
\texttt{servo.compensate\_current} &
\texttt{false} &
boolean &
drive-dependent &
Uses current compensation when sending controller parameters. \\
Duty EMA alpha &
\num{0.001} &
fixed &
fixed &
Very slow estimate of actuator duty cycle. \\
Wattage EMA coefficient &
\(0.003\Delta t\) &
fixed &
fixed &
Slow average power estimate, roughly several minutes. \\
\bottomrule
\end{tabularx}
\end{table*}
...
sensor noise.
\subsubsection{Clutch, Brake, and Position PID}
\begin{table*}[t]
\centering
\caption{Clutch, brake, and position-control parameters.}
\label{tab:pypilot-servo-position}
\begin{tabularx}{\textwidth}{Z{0.25\textwidth}ccZ{0.18\textwidth}Y}
\toprule
Value & Default & Code range & Practical range & Meaning \\
\midrule
\texttt{servo.clutch\_pwm} &
\(\SI{100}{\percent}\) &
\(\SI{10}{\percent}\ldots\SI{100}{\percent}\) &
\(\SI{70}{\percent}\ldots\SI{100}{\percent}\) &
Clutch PWM command. \\
\texttt{servo.use\_brake} &
\texttt{true} &
boolean &
drive-dependent &
Applies brake behavior when idle. \\
\texttt{servo.position.p} &
\num{0.1} &
\(\num{0.01}\ldots\num{1.0}\) &
\(\num{0.05}\ldots\num{0.3}\) &
Rudder-position proportional gain. \\
\texttt{servo.position.i} &
\num{0} &
\(\num{0}\ldots\num{0.1}\) &
\(\num{0}\ldots\num{0.02}\) &
Rudder-position integral gain. \\
\texttt{servo.position.d} &
\num{0.01} &
\(\num{0}\ldots\num{0.1}\) &
\(\num{0}\ldots\num{0.03}\) &
Rudder-position derivative/speed damping. \\
\bottomrule
\end{tabularx}
\end{table*}
...
time constant of \(\SI{2.5}{s}\).
\subsection{Rudder Calibration Parameters}
The rudder sensor model maps a raw normalized sensor reading into degrees:
\[
\delta_r
=
s r
...
n(r_{\min}-r)(r_{\max}-r),
\]
where \(r\) is the raw rudder reading, \(s\) is scale, \(o\) is offset,
and \(n\) is nonlinearity.
\begin{table*}[t]
\centering
\caption{Rudder calibration values.}
\label{tab:pypilot-rudder-calibration}
\begin{tabularx}{\textwidth}{Z{0.25\textwidth}ccZ{0.18\textwidth}Y}
\toprule
Value & Default & Code range & Practical range & Meaning \\
\midrule
\texttt{rudder.range} &
\(45^\circ\) &
\(10^\circ\ldots100^\circ\) &
actual hardover angle &
Maximum absolute rudder angle. \\
\texttt{rudder.offset} &
\num{0} &
not range-limited &
calibration result &
...
\begin{tabularx}{\textwidth}{Z{0.16\textwidth}YZ{0.16\textwidth}}
\toprule
...
Fenix is another open-source marine autopilot, but its architecture and
control philosophy differ substantially from pypilot.  The most important
...
\begin{table*}[t]
\centering
\caption{System-level comparison between pypilot and Fenix.}
...
\text{target rudder}
\rightarrow
\text{feedback actuator positioning}
\rightarrow
\text{PWM and direction pin}.
\]
...
\begin{table*}[t]
\centering
\caption{Actuator-control comparison.}
...
\begin{table*}[t]
\centering
\caption{Commissioning and protocol comparison.}
...
\text{ICM-20948}
+
\text{feedback linear actuator}
+
\text{NMEA/Bluetooth app}.
\]
...
\[
\text{Linux autopilot process}
+
\text{network value server}
+
\text{sensor services}
+
\text{independent servo controller}.
\]
...
\text{heading error}
\rightarrow
\text{target rudder angle}
\rightarrow
\text{feedback linear-actuator loop}.
\end{array}
\]
