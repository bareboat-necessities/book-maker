...
\midrule
Euler & \num{1.2e-2} & \qty{0.3}{ms} \\
RK4   & \num{2.1e-5} & \qty{1.4}{ms} \\
\bottomrule
\end{tabular}
\end{table}
...
Place an SVG file in the same folder, for example \texttt{device.svg}.
The example below is guarded, so the document still builds when the file is
missing.

\begin{figure}[h]
\centering
% Requires: latexmk -lualatex -shell-escape basics.tex
% Requires Inkscape installed if the SVG actually exists.
\IfFileExists{device.svg}{%
  \includesvg[width=0.8\columnwidth]{device}%
}{%
  \fbox{\parbox{0.75\columnwidth}{\centering SVG placeholder\\\texttt{device.svg} not found}}%
}
