...
\begin{table*}[!tbp]
\centering
\caption{GPS, APB navigation, GPS filter, and tacking parameters.}
\label{tab:pypilot-gps-apb-filter-tacking-parameters}
\begin{tabularx}{\textwidth}{Z{0.23\textwidth}c c Z{0.16\textwidth}Y}
\toprule
...
\addlinespace[0.25ex]
\texttt{ap.tack.delay} & \(\SI{0}{s}\) & \(\SI{0}{s}\ldots\SI{60}{s}\) & \(\SI{0}{s}\ldots\SI{10}{s}\) & Delay between tack request and start of tack. \\
\texttt{ap.tack.angle} & \(100^\circ\) & \(10^\circ\ldots180^\circ\) & \(70^\circ\ldots120^\circ\) & Nominal tack angle in compass mode. \\
\texttt{ap.tack.rate} & \(\SI{15}{deg/s}\) & \(\SI{1}{deg/s}\ldots\SI{100}{deg/s}\) & \(\SI{5}{deg/s}\ldots\SI{25}{deg/s}\) & Turn-rate scaling during the tack. \\
\texttt{ap.tack.threshold} & \(\SI{50}{\percent}\) & \(\SI{10}{\percent}\ldots\SI{100}{\percent}\) & \(\SI{40}{\percent}\ldots\SI{70}{\percent}\) & Percent completion at which the tack is considered complete. \\
\texttt{ap.tack.use\_heel} & \texttt{false} & boolean & usually \texttt{false} & Infer tack side from heel. \\
\texttt{ap.tack.use\_wind\_direction} & \texttt{true} & boolean & usually \texttt{true} & Infer tack side from apparent wind direction. \\
\addlinespace[0.6ex]
...
The related servo settings are consolidated in
\cref{tab:pypilot-servo-actuator-parameters}.  The group headers separate
safety limits, output scaling, timing gates, and lower-level calibration
while keeping the shared default/range/meaning columns in one place.
\begin{table*}[!tbp]
\centering
\caption{Servo and actuator parameters.}
\label{tab:pypilot-servo-actuator-parameters}
\begin{tabularx}{\textwidth}{Z{0.23\textwidth}c c Z{0.16\textwidth}Y}
\toprule
Value/gate & Default & Code range & Practical range & Meaning \\
\midrule
\multicolumn{5}{l}{\textit{Safety limits}} \\
\addlinespace[0.25ex]
\texttt{servo.max\_current} & \(\SI{4.5}{A}\) & \(\SI{0}{A}\ldots\SI{50}{A}\) & drive-dependent & Motor current trip limit sent to the controller. \\
\texttt{servo.max\_controller\_temp} & \(\SI{60}{\celsius}\) & \(\SI{45}{\celsius}\ldots\SI{80}{\celsius}\) & \(\SI{55}{\celsius}\ldots\SI{70}{\celsius}\) & Controller over-temperature limit. \\
\texttt{servo.max\_motor\_temp} & \(\SI{60}{\celsius}\) & \(\SI{30}{\celsius}\ldots\SI{80}{\celsius}\) & \(\SI{50}{\celsius}\ldots\SI{70}{\celsius}\) & Motor over-temperature limit. \\
\texttt{servo.faults} & \num{0} & resettable & diagnostic & Persistent fault counter. \\
\addlinespace[0.6ex]
\multicolumn{5}{l}{\textit{Output scaling and pulse shaping}} \\
\addlinespace[0.25ex]
\texttt{servo.gain} & \num{1} & \(\num{-10}\ldots\num{10}\) & \(\num{0.5}\ldots\num{2}\), or \(-1\) if reversed & Multiplies final speed command; negative values reverse the drive direction. \\
\texttt{servo.speed.min} & \(\SI{100}{\percent}\) & \(\SI{0}{\percent}\ldots\SI{100}{\percent}\) & \(\SI{20}{\percent}\ldots\SI{60}{\percent}\) after calibration & Minimum motor speed once a pulse is emitted. \\
\texttt{servo.speed.max} & \(\SI{100}{\percent}\) & \(\SI{0}{\percent}\ldots\SI{100}{\percent}\) & \(\SI{70}{\percent}\ldots\SI{100}{\percent}\) & Maximum motor speed. \\
\texttt{servo.speed\_gain} & \num{0} & \(\num{0}\ldots\num{1}\) & \(\num{0}\ldots\num{0.3}\) & Raises minimum speed as duty cycle increases. \\
\texttt{servo.period} & \(\SI{0.4}{s}\) & \(\SI{0.1}{s}\ldots\SI{1.0}{s}\) & \(\SI{0.25}{s}\ldots\SI{0.6}{s}\) & Pulse/windup period used to avoid tiny actuator movements. \\
\addlinespace[0.6ex]
\multicolumn{5}{l}{\textit{Manual override and command timing gates}} \\
\addlinespace[0.25ex]
Manual override block & \(\SI{0.8}{s}\) & fixed & fixed & After an external command, pilot-generated commands are ignored for this long. \\
Command timeout & \(\SI{1}{s}\) & fixed & fixed & A stale nonzero command is reset to zero. \\
Zero-command resend interval & \(\SI{0.2}{s}\) after \(\SI{1}{s}\) & fixed & fixed & Avoids spamming zero commands while still keeping the link alive. \\
Driver timeout & \(\SI{1}{s}\) & fixed & fixed & If command is nonzero but measured current remains zero, a driver-timeout flag is set. \\
\addlinespace[0.6ex]
